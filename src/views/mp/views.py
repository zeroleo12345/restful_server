...
from models import Platform, Client
    ...
    def post(self, request):
        ...
        def get_reply_msg():
            ...
            if isinstance(msg, SubscribeScanEvent) or isinstance(msg, ScanEvent):
                ...
                assert platform
                user = Client.get(openid=from_user_openid)
                if not user:
                    # user 表记录, 不存在
                    Client.create(openid=from_user_openid, platform_id=platform.id)
                else:
                    # user 表记录, 存在
